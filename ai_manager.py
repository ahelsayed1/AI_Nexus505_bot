...
class SmartAIManager:
    ...
    def _analyze_google_model(self, model_name: str) -> Optional[ModelInfo]:
        """تحليل موديل Google (ترتيب ذكي: الموزة > 4.0 > 3.0)"""
        try:
            model_lower = model_name.lower()
            
            # السماح بجميع موديلات الصور الحديثة
            if "001" in model_lower and "gemini" not in model_lower and "imagen" not in model_lower:
                if "banana" not in model_lower: return None
            ...
            # تحديد الأولويات (الأقل رقم = المحاولة الأولى)
            version = "1.0"
            priority = 100 
            
            if service_type == ServiceType.CHAT:
                if 'gemini-3' in model_lower: priority = 10
                elif 'gemini-2.5' in model_lower: priority = 20
                elif 'gemini-1.5' in model_lower: priority = 50
                elif 'gemini-2.0' in model_lower: priority = 90
                
            elif service_type == ServiceType.IMAGE:
                # 🔥 هنا الترتيب الذكي:
                if 'banana' in model_lower:      priority = 5   # المحاولة 1
                elif 'imagen-4' in model_lower:  priority = 10  # المحاولة 2
                elif 'imagen-3' in model_lower:  priority = 20  # المحاولة 3 (الضمان)
                else: priority = 50
