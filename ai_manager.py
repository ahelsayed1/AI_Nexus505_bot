...
class SmartAIManager:
    ...
    async def _setup_and_discover_google(self):
        ...
        """تحليل موديل Google (محدث لدعم Gemini 3.0 و 2.5)"""
        try:
            model_lower = model_name.lower()
            
            # استبعاد الموديلات التجريبية القديمة جداً أو التي تسبب مشاكل
            if "001" in model_lower or "004" in model_lower: 
                if "gemini-3" not in model_lower and "gemini-2.5" not in model_lower:
                    return None
            ...
            # استخراج الإصدار (يدعم 3 و 3.0 و 2.5)
            version = "1.0"
            priority = 100 # كلما قل الرقم زادت الأهمية
            
            # منطق تحديد الأولويات الجديد (الأقل هو الأهم)
            if service_type == ServiceType.CHAT:
                if 'gemini-3' in model_lower:      # 🔥 الجيل الثالث (الأحدث والأقوى)
                    version = "3.0"
                    priority = 10
                elif 'gemini-2.5' in model_lower:  # 🔥 الجيل 2.5 (ممتاز)
                    version = "2.5"
                    priority = 20
                elif 'gemini-1.5' in model_lower:  # المستقر القديم
                    version = "1.5"
                    priority = 50
                elif 'gemini-2.0' in model_lower:  # ⚠️ الموديل الذي يسبب مشاكل الكوتا
                    version = "2.0"
                    priority = 90 # نضعه في آخر القائمة
                else:
                    priority = 100
                ...
                # تأخير بسيط للنسخ التجريبية
                if 'preview' in model_lower:
                    priority += 2 
                
            elif service_type == ServiceType.IMAGE:
                # دعم Imagen 4 و 3
                if 'imagen-4' in model_lower: priority = 10
