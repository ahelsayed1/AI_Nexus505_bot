...
class SmartAIManager:
    ...
    def _analyze_google_model(self, model_name: str) -> Optional[ModelInfo]:
        """تحليل موديل Google (يدعم Gemini 3.0, 2.5 و Nano Banana)"""
        try:
            model_lower = model_name.lower()
            
            # استبعاد الموديلات القديمة (مع الحفاظ على banana)
            if "001" in model_lower or "004" in model_lower: 
                if "gemini-3" not in model_lower and "gemini-2.5" not in model_lower and "banana" not in model_lower:
                    return None

            # ---------------------------------------------------------
            # 1. تحديد نوع الخدمة (تمت إضافة banana هنا)
            # ---------------------------------------------------------
            if 'gemini' in model_lower and 'tts' not in model_lower and 'audio' not in model_lower:
                service_type = ServiceType.CHAT
            elif 'imagen' in model_lower or 'banana' in model_lower: # ✅ دعم Banana
            ...
            # ---------------------------------------------------------
            # 2. تحديد الأولوية (الأقل = الأهم)
            # ---------------------------------------------------------
            version = "1.0"
            priority = 100 
            
            if service_type == ServiceType.CHAT:
                if 'gemini-3' in model_lower:      version = "3.0"; priority = 10
                elif 'gemini-2.5' in model_lower:  version = "2.5"; priority = 20
                elif 'gemini-1.5' in model_lower:  version = "1.5"; priority = 50
                elif 'gemini-2.0' in model_lower:  version = "2.0"; priority = 90
                
                if 'flash' in model_lower: priority -= 5
                if 'pro' in model_lower: priority -= 2
                
            elif service_type == ServiceType.IMAGE:
                # ✅ هنا التعديل: Nano Banana تأخذ المركز الأول
                if 'banana' in model_lower:      priority = 5  # 🔥 الأسرع والأول
                elif 'imagen-4' in model_lower:  priority = 10 # الجودة العالية
                elif 'imagen-3' in model_lower:  priority = 20
                else: priority = 50
                
            elif service_type == ServiceType.VIDEO:
                if 'veo-3' in model_lower: priority = 10
                else: priority = 50
    ...
    def _log_discovery_results(self):
        ...
        for provider_name, config in self.providers.items():
            if not config.enabled:
                logger.info(f"\n🔹 {provider_name.value.upper()}: ❌ غير مفعل")
                continue
    ...
    async def _generate_image_google(self, model_name: str, prompt: str) -> str:
        """توليد صورة عبر الاتصال المباشر (REST API) لتجاوز مشاكل المكتبة"""
        try:
            logger.info(f"🎨 Google Direct: جاري التوليد باستخدام {model_name}...")
            
            if not os.path.exists("downloads"):
                os.makedirs("downloads")
            filename = f"downloads/img_{int(time.time())}.png"

            # 1. إعداد رابط API المباشر
            ...
            # 3. الاتصال المباشر (تجاوز المكتبة)
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers={'Content-Type': 'application/json'}) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"خطأ من جوجل ({response.status}): {error_text}")
                    
                    result = await response.json()
                    
                    # 4. استخراج الصورة (تأتي مشفرة Base64)
                    try:
                        # الصيغة الشائعة لـ Imagen
                        predictions = result.get('predictions', [])
                        if not predictions:
                            raise Exception("لم يتم استلام أي تنبؤات من جوجل")
                        
                        b64_data = predictions[0].get('bytesBase64Encoded')
                        if not b64_data:
                            # محاولة صيغة أخرى قد تستخدمها Banana
                            b64_data = predictions[0].get('image', {}).get('bytesBase64Encoded')
                            
                        if not b64_data:
                            raise Exception(f"صيغة رد غير معروفة: {str(result)[:100]}")
                        ...
                        raise Exception(f"فشل معالجة رد الصورة: {parse_error}")

        except Exception as e:
            logger.error(f"❌ فشل Google Direct: {e}")
            
            # محاولة Fallback لـ OpenAI
            openai_config = self.providers[Provider.OPENAI]
            if openai_config.enabled:
                return await self._generate_image_openai("dall-e-3", prompt)
            
            raise Exception(f"فشل التوليد: {e}")
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if "artifacts" in result and len(result["artifacts"]) > 0:
                            ...
                            # مؤقتاً نعيد رابط وهمي مع تنبيه
