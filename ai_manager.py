...
class SmartAIManager:
    ...
    async def _generate_image_google(self, model_name: str, prompt: str) -> str:
        """توليد صورة باستخدام Google Imagen أو Nano Banana (نسخة الإجبار)"""
        try:
            logger.info(f"🎨 Google Image: جاري التوليد باستخدام {model_name}...")
            ...
            def run_imagen():
                # محاولة استيراد كلاس الصور بأكثر من طريقة لضمان العمل
                ImageModel = None
                try:
                    # الطريقة المباشرة
                    ImageModel = genai.ImageGenerationModel
                except AttributeError:
                    try:
                        # طريقة الاستيراد الفرعي
                        from google.generativeai import ImageGenerationModel
                        ImageModel = ImageGenerationModel
                    except ImportError:
                        raise Exception("كلاس ImageGenerationModel غير موجود في المكتبة")

                if not ImageModel:
                    raise Exception("تعذر الوصول لأداة الصور")

                # تهيئة الموديل
                image_model = ImageModel(model_name)
            ...
            file_path = await asyncio.to_thread(run_imagen)
            return file_path

        except Exception as e:
            logger.error(f"❌ فشل Google Image: {e}")
            # Fallback لـ OpenAI
            openai_config = self.providers[Provider.OPENAI]
            if openai_config.enabled:
                logger.info("⚠️ التحويل إلى OpenAI DALL-E كبديل...")
                return await self._generate_image_openai("dall-e-3", prompt)
            
            # تمرير الخطأ الأصلي
            raise Exception(f"فشل التوليد بالموزة: {e}")
