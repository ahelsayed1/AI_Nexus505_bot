...
class SmartAIManager:
    ...
    def _log_discovery_results(self):
        """تسجيل نتائج الاكتشاف (عرض القائمة كاملة بدون إخفاء)"""
        logger.info("=" * 50)
        ...
        for provider_name, config in self.providers.items():
            ...
            for service_type, models in config.discovered_models.items():
                if models:
                    ...
                    # ✅ التعديل هنا: حذفنا [:3] لنعرض كل شيء
                    for i, model in enumerate(models):
                        # نضع نجمة ⭐ أمام أول 5 موديلات (الأقوى)
                        status = "⭐" if i < 5 else "  "
            ...
            for provider_config in providers:
                try:
                    ...
                    response = await self._execute_with_fallback(
                        provider, ServiceType.CHAT, execute_chat, max_retries=5  # ✅ كانت 2 اجعلها 5
                    )
